...
# could put a number of times opt_found is called in the lambda
  ...
  def test_arity_0
    ...
    assert_equal(["r9t-3r.csv"], od.args, "Failed with -d defined as second arg.")
    od     = parser.parse(%w{-d arg1 arg2})
    assert_equal(%w{arg1 arg2}, od.args, "Failed with -d defined as second arg.")
  ...
  def test_command_mode
    cmds = %w{edit update history}
    app_opt = Option.new(:names => %w(-d))
    cmd_update_opt  = Option.new(:flag, :names => "-r")
    ...
    commands = {
      ...
      "update"  => OptionParser.new(cmd_update_opt),
